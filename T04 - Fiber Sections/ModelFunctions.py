...
def getSections():
    ...
    h = 600*mm
    w = 400*mm
    verticies = np.array([-h/2, w/2, -h/2, -w/2, h/2, -w/2, h/2, w/2])
    
    # Define Rebar Info
    rebarZ = np.array([-150, 0, 150])*mm
    rebarY = np.array([-225, -150, 150, 225])*mm
    ...
    op.patch('quad', 1, NfibeZ, NfibeY, *verticies)
    ...
    #  patch('quad', matTag, numSubdivIJ, numSubdivJK, *crdsI, *crdsJ, *crdsK, *crdsL)
    op.patch('quad', 3, NfibeZ, NfibeY, *verticies)
    ...
    op.beamIntegration('Lobatto', 10, 1, 4)    
...
def buildModel():
    ...
    op.element('forceBeamColumn', 1, *[1,2], 1, 10, '-iter', 30, 1e-12)
    op.element('forceBeamColumn', 2, *[2,3], 1, 10, '-iter', 30, 1e-12)
    op.element('forceBeamColumn', 3, *[3,4], 1, 10, '-iter', 30, 1e-12)
